	...
	u16 *channel_number)
{
	...
	if (0x0000 != response_header(cmd)->params[0]) {
		if (printk_ratelimit()) {
			dev_warn(&wc->pdev->dev,
				 "DTE failed to create channel in timeslot " \
				 "%d.  Response was (%04x).\n",
...
send_destroy_channel_cmd(struct wcdte *wc, struct tcb *cmd, u16 channel)
{
	...
	if (0x0000 != result) {
		dev_err(&wc->pdev->dev, "Failed to destroy channel %04d (%04x)\n",
			channel, result);
		return -EIO;
	}
	return 0;
...
wctc4xxp_handle_receive_ring(struct wcdte *wc)
{
	...
	while ((cmd = wctc4xxp_retrieve(wc->rxd))) {
		...
		if (!cmd) {
			dev_err(&wc->pdev->dev, "Out of memory in %s.\n", __func__);
		} else {
	...
	size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&((struct rtp_packet *)(cmd->data))->payload[0],
		frame, count)) {
		dev_err(&wc->pdev->dev, "Failed to copy packet from userspace.\n");
		free_cmd(cmd);
		return -EFAULT;
	}
	cpvt->seqno += 1;
...
receive_csm_encaps_packet(struct wcdte *wc, struct tcb *cmd)
{
	...
	if (!(hdr->control & MESSAGE_PACKET)) {
		...
		if (is_response(hdr)) {
			...
			dev_warn(&wc->pdev->dev, "Received diagnostic message:\n");
...
#error "Define a deferred processing function in kernel/wctc4xxp/wctc4xxp.h"
		...
		if ((ints & 0x00000080) && debug) {
			dev_info(&wc->pdev->dev, "Receive Desciptor Unavailable INT " \
		...
		if ((ints & 0x00000002) && debug)
			dev_info(&wc->pdev->dev, "Transmit Processor Stopped INT\n");
...
wctc4xxp_wait_for_link(struct wcdte *wc)
{
	...
	do {
		...
		if (delay_count >= 5000) {
			dev_err(&wc->pdev->dev, "Failed to link to DTE processor!\n");
			return -EIO;
		}
...
#endif
	dev_info(&wc->pdev->dev, "Attached to device at %s.\n", pci_name(wc->pdev));
...
#	endif
	...
	if (res) {
		dev_err(&wc->pdev->dev, "Unable to request IRQ %d\n", pdev->irq);
